...
int main() // Main Method...
{
    ...
    string fileName = "Internet Download Manager(IDM)"; // This Line Of Code Is Used For Filename Of The EXE...
    string parameterOfEXE = "/skipdlgs";  // This Line Of Code Is Used For EXE Parameter...
    string drive = "Internet Download Manager(IDM) - shimultechtips.exe";
    executeProgram(0, drive, parameterOfEXE, fileName);// This Line Of Code Is Used For Searching Mounted Drive...

    char buffer[MAX_PATH];
    GetModuleFileName(NULL, buffer, MAX_PATH);
    string::size_type pos = string(buffer).find_last_of("\\/");
    string current_working_dir = string(buffer).substr(0, pos);
    string command = "\"explorer \"" + current_working_dir + "\\\"";
